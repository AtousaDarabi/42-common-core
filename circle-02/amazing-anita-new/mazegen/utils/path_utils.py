...
def get_direction_letter(start: Tuple[int, int], end: Tuple[int, int]) -> str:
    """!
    @brief Determines the cardinal direction (N, E, S, W) between two adjacent cells.
    ...
    @return A string character representing the direction ('N', 'E', 'S', or
            'W'), or "" if `start`/`end` are not exactly one adjacent step apart.
    @details Maps the (dx, dy) coordinate difference to a single compass letter.
    """
    dx = end[0] - start[0]
    dy = end[1] - start[1]
...
def format_path(path_coords: List[Tuple[int, int]]) -> str:
    ...
    @details Iterates through consecutive points to determine movement and joins them.
    """
    if not path_coords:
        return ""
